# #!/usr/bin/env python3
# """
# STG-NF 자동 소프 실행 스크립트.
#   - 진단 → 클러스터링 → Fisher 병합까지 원샷 파이프라인
# 예시:
# python auto_soup.py --reference_args experiments/.../args.json \\
#   --checkpoints ckpt1 ckpt2 ckpt3 --output_dir auto_soup_out
# """
...
#     mask_name: str,
# ) -> Dict[str, float]:
#     models_eval, _, _ = load_models_and_fishers(
#         checkpoint_paths=[str(ckpt_path)],
#         fisher_paths=None,
...
#             _, nll = model(
#                 sample.float(),
#                 label=torch.ones(sample.shape[0], device=device),
#                 score=clip_score,
#             )
...
#     prob_np = probs.cpu().detach().numpy().squeeze().copy(order="C")
#     auc, scores_np, labels_np, _ = score_dataset(prob_np, dataset_test.metadata, args=ref_args)
#     metrics = {
#         "roc_auc": float(auc),
#         "auc": float(auc),
...
#     """Robust torch.load that prefers weights_only but falls back safely for older checkpoints."""
...
#     except TypeError:
#         pass  # older torch that lacks weights_only
...
#         unique_paths = list(dict.fromkeys(result_paths))
...
#                 args.mask_name,
#             )
#             eval_results[rp] = metrics
#             print(f"[EVAL] {rp} -> {json.dumps(metrics)}")
