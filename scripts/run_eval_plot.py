...
def main():
    ...
    results["base"]["val_nll_mean"] = float(base_val.mean())
    ...
    for name, path in zip(model_names, model_paths):
        ...
        model_result["val"]["nll_mean"] = float(val_nll.mean()) if val_nll.size else 0.0
        ...
        for df_name, df_sids in df_sets.items():
            ...
            model_result["df"][df_name] = {
                "nll_mean": float(df_nll.mean()) if df_nll.size else 0.0,
                "delta_df": float(df_nll.mean() - base_df.mean()) if df_nll.size else 0.0,
            }
        ...
        test_nll = eval_nll_on_indices(
            ...
            batch_size=args.batch_size_eval,
            num_workers=args.num_workers,
            use_conf_score=args.model_confidence,
        )
